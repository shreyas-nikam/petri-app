...
def _worker_eval(log_q: mp.Queue, instructions: list[str], run_dir: str, api_key: str):
    ...
    if not api_key:
        log_q.put(("ERR", f"Missing API key in env var"))
        log_q.put(("DONE", 1))
        return

    log_q.put(("OUT", "Creating target agent...\n"))
    log_q.put(("OUT", "Creating auditor agent...\n"))
    log_q.put(("OUT", "Creating judge agent...\n"))

    MODEL = "openai/gpt-4o-mini"
    try:
        target_model = get_model(MODEL, api_key=api_key)
        auditor_model = get_model(
            MODEL, api_key=api_key, config=GenerateConfig(
                max_tokens=16_000, reasoning_tokens=8_000)
        )
    ...
    log_q.put(
        ("OUT", f"[info] Instructions: {len(instructions)} line(s) provided\n"))
    log_q.put(("OUT", f"[info] Instructions: {instructions}\n"))
    log_q.put(("OUT", f"[info] Running eval...\n"))

    # Run eval
    try:

        if st.session_state.mode == "Choose from predefined probes":
            cached_results_lookup = json.load(
                open("cached_results_lookup.json"))
